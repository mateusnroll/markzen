...
describe('spec 0001 repository verification contract', () => {
  ...
  test('spec 0009 AC62 AC67: CSV uses closed typed intents and local deterministic test boundaries', async () => {
    ...
    expect(contracts).toContain('readonly kind: EditorDocumentKind')
    expect(contracts).toContain("readonly kind: 'raster'")
    expect(contracts).toContain("readonly kind: 'external'")
