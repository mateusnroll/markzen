...
- AC20: Given a packaged renderer response, when its Content Security Policy is inspected, then it contains `default-src 'none'; script-src 'self'; style-src 'self'; img-src 'self' markzen-asset:; font-src 'self'; connect-src 'none'; media-src 'none'; object-src 'none'; child-src 'none'; frame-src 'none'; worker-src 'none'; base-uri 'none'; form-action 'none'; frame-ancestors 'none'`; direct `data:`, `blob:`, and remote image admission, inline/evaluated script, and development-only Vite allowances are absent. Specs 0005–0006 admit validated image bytes only through the separately constrained `markzen-asset:` capability.
