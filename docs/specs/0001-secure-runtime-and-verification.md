...
- Future local-resource rendering must use a separate main-registered safe protocol with opaque, scoped capabilities. Raw `file:` renderer access, arbitrary path URLs, and disabling `webSecurity` are forbidden.
...
- AC19: Given a packaged Markzen renderer, when its document or subresource is requested, then it loads only from the fixed `markzen://app` bundle allowlist; `file:`, traversal, encoded traversal, unknown hosts, and unknown bundled paths are rejected.
- AC20: Given a packaged renderer response, when its Content Security Policy is inspected, then it contains `default-src 'none'; script-src 'self'; style-src 'self'; img-src 'self' data:; font-src 'self'; connect-src 'none'; media-src 'none'; object-src 'none'; child-src 'none'; frame-src 'none'; worker-src 'none'; base-uri 'none'; form-action 'none'; frame-ancestors 'none'`; inline/evaluated script and development-only Vite allowances are absent.
