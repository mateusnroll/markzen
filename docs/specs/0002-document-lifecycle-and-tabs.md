...
- AC167: Given any Markdown image source without a live local exact-resource capability from spec 0005, when it renders, then a selectable placeholder exposes safe alt text without assigning the authored source to a fetch-capable DOM attribute and causes no network, `file:`, unapproved custom-protocol, `data:`, decode, navigation, or filesystem request. Spec 0005 owns the only current local-image capability and spec 0006 owns any future remote or embedded load action.
...
- AC166 is the permanent no-ambient-navigation floor and has been narrowed and reapproved for milestone 0004's explicit guarded link actions; its negative tests must exercise non-action parsing, rendering, focus, plain-click, and programmatic-change paths rather than forbid later Approved user intent. AC167 is the permanent no-ambient-image-load floor, narrowed and reapproved for spec 0005's explicit local exact-resource capability; its negative tests cover parsing, rendering, focus, remote/data sources, blocked local sources, and programmatic changes without a live capability.
