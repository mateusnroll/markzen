...
- AC166: Given any Markdown link, when it parses, renders, receives focus, is plain-clicked, or changes without an explicit opening action owned by a later Approved spec, then it causes no navigation, popup, system-handler call, IPC request, or network request; milestone 0004 owns the only current explicit safe and confirmed external-opening actions while preserving this no-ambient-navigation floor.
- AC167: Given any Markdown image source without a live exact-resource capability from spec 0005 or a current-generation remote/embedded capability from spec 0006, when it renders, then a selectable placeholder exposes safe alt text without assigning the authored source to a fetch-capable DOM attribute and causes no network, `file:`, unapproved custom-protocol, raw `data:`, decode, navigation, or filesystem request. Spec 0006 narrows this floor only for an explicit current-node Load/Retry intent or strict embedded-data resolution whose validated bytes are returned through a revocable `markzen-asset:` token.
...
- AC166 is the permanent no-ambient-navigation floor and has been narrowed and reapproved for milestone 0004's explicit guarded link actions; its negative tests must exercise non-action parsing, rendering, focus, plain-click, and programmatic-change paths rather than forbid later Approved user intent. AC167 is the permanent no-ambient-image-load floor, narrowed and reapproved for spec 0005's local exact-resource capability and spec 0006's explicit current-generation remote Load/Retry plus strict embedded-data resolution; its negative tests cover parsing, rendering, focus, blocked sources, and programmatic changes without a live capability.
