...
**Origin:** Promotes the first ordered item from `BACKLOG.md` and
...
and narrowed ADR updates. Its proposed removal of the header-row toggle was
rejected because the user explicitly selected that product behavior.
...
- Editable CSV input is at most 10 MiB of source bytes after an optional UTF-8
  BOM, 10,000 records, 1,000 fields in one record, 100,000 total fields, and
  1 MiB of decoded text in one field. Parsing stops at the first exceeded bound
  and selects exact preservation rather than allocating the complete model.
...
- Clipboard matrices reuse the same bounded CSV parser and serializer with an
  explicit tab delimiter rather than introducing a second quoting codec. Paste
  adds only clipboard-specific expansion and editable-bound policy.
...
- Grid rendering is windowed in both axes so DOM size follows the viewport, not
  the document. Accessibility exposes the complete logical row and column
  counts and indexes even when most cells are not mounted.
...
- AC15: Given the selected delimiter, when the complete file is parsed, then
  ...
  spaces, empty leading/interior/trailing fields, LF and CRLF record
...
- AC18: Given input crosses any byte, record, per-record field, total-field, or
  ...
  exceeded limit; a source above the existing 32 MiB outer document-transfer
  bound instead fails before renderer transfer with an accessible error and
  retains no tab ownership.
- AC19: Given preservation from AC17 or AC18, then Save performs no write,
  rename-only preserves the original bytes, Save As may copy the exact original
  bytes, Find and CSV mutation controls are unavailable, and the ordinary
  conflict, close, and ownership behavior remains intact.
...
- AC23: Given an edited CSV document, when serialized, then it uses the
  ...
  LF or CRLF record-separator convention, and source presence or absence of a
  final record separator; a new CSV uses UTF-8 without BOM, LF, and no final
  record separator.
...
- AC26: Given an edited semantic matrix, when the production serializer writes
  it, then its bytes equal an independently authored canonical golden; reparsing
  that golden produces the same matrix and dialect metadata except for
  intentionally canonical field quoting.
- AC27: Given formula-looking, HTML-looking, URL-looking, control-like, Unicode,
  ...
  when rendered, copied, edited, saved, and reopened, then it remains literal
  field data and never executes, navigates, fetches, gains rich marks, or
  changes type.
...
- AC32: Given a CSV tab is switched away from and back to, then its header
  toggle, active cell, rectangular selection, editing caret, undo history, and
  scroll position are restored; reopening the file in a later application
  session starts with the header toggle on.
...
- AC35: Given a cell is clicked, focused through grid navigation, or selected
  ...
  reduced motion, and announces its row, column, optional header name, and
  literal value.
- AC36: Given the active cell, when Enter, F2, double-click, or a printable key
  outside IME composition begins editing, then a plainly labeled text editor
  owns the cell; a printable key replaces its selected value while Enter, F2,
  or double-click preserves the value and caret placement.
...
- AC39: Given the grid at 480×320, 200% zoom, forced colors, reduced motion, or
  ...
  reachable and distinguishable without color or animation alone.
...
- AC41: Given a selected range, when Cmd/Ctrl+C is invoked, then one
  ...
  LF inside a field; content and dirty state do not change.
...
- AC60: Given overlapping load, parse, delimiter detection, search, clipboard,
  save, watcher, preview, tab-switch, or close work, then every completion
  checks its tab owner, document kind, operation generation, and captured
  revision before commit; stale work cannot change another kind, tab, path,
  matrix, selection, header state, or active focus.
- AC61: Given CSV field text containing HTML, Markdown, URLs, image syntax,
  ...
  network request.
- AC62: Given the expanded renderer/preload boundary, then New CSV and CSV
  document intents use closed enums and existing sender-derived tab/window
  authority; renderer-supplied kind, delimiter, title, matrix, FileKey, path, or
...
- AC64: Given the CSV grid uses windowing, then no more than 600 data
  ...
  `aria-colcount`, `aria-rowindex`, and `aria-colindex` describe the complete
  matrix and roving focus remains singular across virtualization.
...
| AC11–AC17 | Node | Playwright-vs-vite representative dialects and preservation |
| AC18 | Shell smoke | Node exact-bound parser tests |
| AC19–AC20 | Node | Playwright-vs-vite preservation presentation |
| AC21–AC28 | Node | Shell smoke newline/BOM and rename-only round trip |
...
| AC62 | Static | Node and shell forged/stale/kind-mismatch negatives |
